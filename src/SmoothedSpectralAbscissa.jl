...
- `grad::Union{Nothing,Matrix{<:Real}}`: `nothing` to skip the gradient computation. Otherwise should be a matrix of the same size as A, that will contain ``\\frac{\\partial\\; \\text{SSA}(A)}{\\partial\\; A_{i,j}}``
- `PQ::SSAAlloc` : Pre-allocated matrices for iterative computations. Can be generated by `SSAAlloc(size(A,1))`
...
# Returns
- `ssa<:Real`: this is ``\\text{SSA}_\\varepsilon(A)``
"""
